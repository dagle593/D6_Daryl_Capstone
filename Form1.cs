...
namespace TempForms
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void updateButton1_Click(object sender, EventArgs e)
        {
            ...
            ResponseGen responseGen = new ResponseGen(operations.RetResultingData);
    ...
    {
        System.Data.DataTable ResultingData = new System.Data.DataTable();
        //tele bot
        private TelegramBotClient botClient;

        public ResponseGen(System.Data.DataTable dataTable)
        {
            ResultingData = dataTable;
        ...
        private async void SendMessageToTelegram()
        {
            try
            {
                ...
                string tableString = DataTableToString(ResultingData);

                // Send the message
                await botClient.SendTextMessageAsync(chatId, tableString);
            }
            catch (Exception ex)
            {
        ...
        private string DataTableToString(System.Data.DataTable dataTable)
        {
            ...
            {
                string operation = row.Field<string>("Operations");
                tableString.AppendLine($"{operation}:");

                // Loop through each day column
                foreach (DataColumn column in dataTable.Columns)
                {
                    if (column.ColumnName != "Operations")
                    {
                        string day = FormatDay(column.ColumnName);
                        double? value = row.Field<double?>(column);
                        tableString.AppendLine($"{day} : {FormatValue(value)}");
                    }
                }

                tableString.AppendLine(); // Add a line break between operations
            }
        ...
        {
            // Format day names as needed, e.g., abbreviate or pad with spaces
            // Example: Wednesday -> Wed
            return day.Substring(0, 3); // Take the first 3 characters as an example
        }

        private string FormatValue(double? value)
        {
            // Format values as needed, e.g., round to 2 decimal places
    ...
    {
        protected List<string> opList = new List<string>();
        protected readonly string[] opNames = new string[] { "AVERAGE", "MINIMUM", "MAXIMUM" };
        protected System.Data.DataTable filteredData = new System.Data.DataTable();

        protected Tuple<double?, Dictionary<string,double>> AverageOp()
        {
            double? average = null;
            Dictionary<string, double> averagesRow = new Dictionary<string, double>();
            ...
            foreach (DataColumn column in filteredData.Columns)
            {
                if (column.ColumnName != "Time")
                {
                    ...
                    average = count > 0 ? Math.Round(sum / count, 2) : 0;
                    averagesRow[column.ColumnName] = average.Value;
                }
            }

            return Tuple.Create(average, averagesRow);

        }

        protected Tuple<double?, Dictionary<string, double>> MinOp()
        {
            double? resMin = null;
            Dictionary<string, double> minValRow = new Dictionary<string, double>();

            foreach (DataColumn column in filteredData.Columns)
            {
                if (column.ColumnName != "Time")
                {
                    resMin = double.MaxValue;
                    ...
                    foreach (DataRow row in filteredData.Rows)
                    {
                        if (row[column] != DBNull.Value)
                        {
                            ...
                            resMin = Math.Min(currValue, resMin ?? currValue);
                        }
                    }

                    minValRow[column.ColumnName] = Math.Round(resMin.Value, 2);

                }
            }

            return Tuple.Create(resMin, minValRow);


        }

        protected Tuple<double?, Dictionary<string, double>> MaxOp()
        {
            double? resMax = null;
            Dictionary<string, double> maxValRow = new Dictionary<string, double>();

            foreach (DataColumn column in filteredData.Columns)
            {
                if (column.ColumnName != "Time")
                {
                    resMax = double.MinValue;
                    ...
                    foreach (DataRow row in filteredData.Rows)
                    {
                        ...
                        {
                            double currValue = Convert.ToDouble(row[column]);
                            resMax = Math.Max(currValue, resMax ?? currValue);
                        }
                    }

                    maxValRow[column.ColumnName] = Math.Round(resMax.Value, 2);

                }
            }

            return Tuple.Create(resMax, maxValRow);
        }
    }

    

    public class Operations : OperationFunctions
    {
        ...
        private new void AverageOp()
        {
            // Add a single row with columns for each selected day in the averagesTable
            var aveResult = base.AverageOp();
            //double? average = aveResult.Item1;
            Dictionary<string,double> aveValues = aveResult.Item2;

            DataRow averagesRow = ResultingData.NewRow();
            foreach (var kvp in aveValues)
            {
                averagesRow[kvp.Key] = kvp.Value;
            }
        ...
        private new void MinOp()
        {
            // Add a single row with columns for each selected day in the minimumTable
            var minResult = base.MinOp();
            //double? minimum = minResult.Item1;
            Dictionary<string, double> minValues = minResult.Item2;

            DataRow minValRow = ResultingData.NewRow();
            foreach (var kvp in minValues)
            {
                minValRow[kvp.Key] = kvp.Value;
            }
        ...
        private new void MaxOp()
        {
            // Add a single row with columns for each selected day in the minimumTable
            var maxResult = base.MaxOp();
            //double? minimum = minResult.Item1;
            Dictionary<string, double> maxValues = maxResult.Item2;

            DataRow maxValRow = ResultingData.NewRow();
            foreach (var kvp in maxValues)
            {
                maxValRow[kvp.Key] = kvp.Value;
            }
        ...
        public System.Data.DataTable FilteredData
        {
